...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            # mm_token_type_ids (Qwen3.5) is per-token like input_ids/attention_mask, not a
            # per-image blob like pixel_values/image_grid_thw — it must be padded to
            # max_prompt_length and batched alongside them, not dumped into multi_modal_inputs
            # where extract_multi_modal_inputs() would torch.cat it across samples of
            # different sequence lengths and crash.
        ...
        if mm_token_type_ids is not None:
            # same original length as input_ids, so the identical pad/truncate op keeps it aligned
            ...
            row_dict["mm_token_type_ids"] = mm_token_type_ids[0]
