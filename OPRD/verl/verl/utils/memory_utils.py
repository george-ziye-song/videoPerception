...
def actor_update_mem_profile_enabled() -> bool:
    return os.getenv("ACTOR_UPDATE_MEM_PROFILE", "0").lower() in ("1", "true", "yes")


@contextmanager
def profile_actor_update_memory():
    """Profile peak GPU memory during actor update when ``ACTOR_UPDATE_MEM_PROFILE=1``.

    Set the env var before launch to compare configs (e.g. rep-only vs top-k). Metrics are
    all-reduced with MAX across ranks so logs reflect the worst GPU.
    """
    if not actor_update_mem_profile_enabled():
        yield None
        return
    ...
    device.synchronize()
    device.reset_peak_memory_stats()
    mem_before = device.memory_allocated()
    metrics: dict[str, float] = {}
    try:
        yield metrics
    finally:
        device.synchronize()
        peak_alloc = device.max_memory_allocated()
        peak_reserved = device.max_memory_reserved()
        delta_peak = peak_alloc - mem_before

        if dist.is_available() and dist.is_initialized():
            stats = torch.tensor(
                [peak_alloc, peak_reserved, delta_peak],
                dtype=torch.float64,
                device=device.current_device(),
            )
            dist.all_reduce(stats, op=dist.ReduceOp.MAX)
            peak_alloc, peak_reserved, delta_peak = stats.tolist()

        gb = 1024**3
        metrics.update(
            {
                "mem/actor_update_peak_alloc_GB": peak_alloc / gb,
                "mem/actor_update_peak_reserved_GB": peak_reserved / gb,
                "mem/actor_update_delta_peak_GB": delta_peak / gb,
            }
        )
        if dist.is_available() and dist.is_initialized() and dist.get_rank() == 0:
            logger.info(
                "Actor update memory profile (max across ranks): "
                f"peak_alloc={peak_alloc / gb:.3f} GB, "
                f"peak_reserved={peak_reserved / gb:.3f} GB, "
                f"delta_peak={delta_peak / gb:.3f} GB"
            )
