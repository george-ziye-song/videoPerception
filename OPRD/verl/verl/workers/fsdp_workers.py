    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if has_remote_code:
                ...
                # Prefer AutoModelForImageTextToText when a model is registered under both: vLLM's own
                # ConditionalGeneration implementation (and its hf_to_vllm_mapper weight-name remapping)
                # is built for the ImageTextToText parameter layout (e.g. `model.language_model.*`), not
                # the bare AutoModelForCausalLM layout (`model.layers.*`), so picking CausalLM first would
                # break FSDP -> vLLM weight sync for natively-multimodal architectures like Qwen3.5.
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Bug fix (2026-07-28): AutoModelForCausalLM.from_pretrained on a natively-multimodal
            # checkpoint (e.g. Qwen3.5) silently loads the text-only class and drops the vision
            # tower entirely (confirmed empirically: resulting config has no vision_config) --
            # the reward model would score/distill from text alone, never seeing the actual video
            # input. Mirrors the exact fix already applied to ActorRolloutRefWorker's own model
            # loading (see this file's `_build_model_optimizer`, ~line 355-379) for the same
            # reason, just never carried over to RewardModelWorker.
    ...
    def _forward_micro_batch(
        ...
        with torch.no_grad(), torch.autocast(device_type=get_device_name(), dtype=torch.bfloat16):
            ...
            # Multimodal support (2026-07-28): mirrors dp_actor.py's DataParallelPPOActor.
            # _forward_micro_batch, which already does exactly this for the actor/ref workers.
            # rm_data.batch["multi_modal_inputs"] (set in compute_rm_score) is a plain per-row
            # Python list assigned onto a TensorDict key matching the batch size, so tensordict
            # auto-wraps it as a NonTensorStack -- rearrange_micro_batches/.split() then slice it
            # row-aligned along with input_ids/position_ids/etc "for free", same as any other
            # per-row key, no manual index bookkeeping needed. The ONE thing that varies is the
            # wrapper shape after slicing: a multi-row micro-batch keeps a NonTensorStack
            # (.tolist() -> list[dict]), but a micro-batch of exactly 1 row can collapse to a bare
            # NonTensorData (.tolist() -> a single dict, not list-wrapped) -- confirmed by direct
            # testing against this tensordict version, not assumed. Normalize both to list[dict].
            multi_modal_inputs = {}
            mm_data = micro_batch.get("multi_modal_inputs", None)
            if mm_data is not None:
                from verl.utils.model import extract_multi_modal_inputs

                if hasattr(mm_data, "tolist"):
                    mm_data = mm_data.tolist()
                if isinstance(mm_data, dict):
                    mm_data = [mm_data]
                multi_modal_inputs = extract_multi_modal_inputs(mm_data)
            mm_token_type_ids = micro_batch.get("mm_token_type_ids", None)
            if mm_token_type_ids is not None and mm_token_type_ids.shape[-1] < seqlen:
                # mm_token_type_ids is built at dataset time over the PROMPT only (the response
                # doesn't exist yet then, and is never visual content anyway) -- but input_ids
                # here is the full prompt+response rollout sequence, longer by response_length.
                # Extend with 0 ("text") for the response suffix so it's the same [batch, seqlen]
                # shape as input_ids/attention_mask before the shared rmpad `indices` (computed
                # from THIS full-length attention_mask) gather from it below -- confirmed via a
                # real crash (CUDA device-side assert, "scatter gather kernel index out of
                # bounds") that using the un-padded, prompt-only-length tensor with indices sized
                # for the longer sequence overruns it.
                pad_len = seqlen - mm_token_type_ids.shape[-1]
            ...
            if self.use_remove_padding:
                ...
                # Multimodal support (2026-07-28): same remove-padding op as position_ids above,
                # using the same `indices` -- mm_token_type_ids is per-token like input_ids, not
                # per-image like multi_modal_inputs (see dp_actor.py's identical handling).
                mm_token_type_ids_rmpad = None
                if mm_token_type_ids is not None:
                    mm_token_type_ids_rmpad = index_first_axis(
                        rearrange(mm_token_type_ids.unsqueeze(-1), "b s ... -> (b s) ..."), indices
                    ).transpose(0, 1)

                # Memory optimization (2026-07-28): by default the model computes full-vocab
                # logits for EVERY position (HF's logits_to_keep=0), but every consumer of
                # logits_rmpad below (rm_log_probs, teacher_entropy, top-k log-probs) ends up
                # sliced down to just the response positions (`[:, -response_length-1:-1]`,
                # see ~130 lines below) before being used for anything -- confirmed by reading
                # every consumer in this function, not assumed. For a ~16.5K-token multimodal
                # sample with vocab_size=248320, materializing the full (total_nnz, vocab)
                # logits tensor is ~7.6GiB bf16, almost all of it discarded immediately -- this
                # was the dominant contributor to a real, confirmed-clean (no other GPU tenants,
                # no other config knob helped, including rep_distillation_only=True which does
                # NOT gate this at all) OOM on a 24GB card. Restricting logits_to_keep to exactly
                # the response-position mask -- built via the same index_first_axis/indices
                # alignment already used for position_ids/mm_token_type_ids above -- computes
                # the mathematically identical result (same positions everything downstream
                # already reduces to) for a fraction of the memory. Left disabled (falls back to
                # the old, unoptimized, already-correct full computation) for Ulysses SP (not
                # used by this worker at all, see the use_ulysses_sp branch below) and for the
                # top-k/OPD path (student_top_k_ids is not None, not exercised by our config --
                # its full_student_top_k_ids/_compute_teacher_top_k_log_probs logic further down
                # assumes full-size logits_rmpad and was left untouched rather than threading
                # this truncation through an unexercised branch).
                use_logits_to_keep = not self.use_ulysses_sp and student_top_k_ids is None
                if use_logits_to_keep:
                    keep_mask = torch.zeros(batch_size, seqlen, dtype=torch.bool, device=input_ids.device)
                    keep_mask[:, -response_length - 1 : -1] = True
                    keep_mask_rmpad = index_first_axis(
                        rearrange(keep_mask.unsqueeze(-1), "b s ... -> (b s) ..."), indices
                    ).squeeze(-1)  # (total_nnz,) bool
                    indices_for_logits = indices[keep_mask_rmpad]
                else:
                    keep_mask_rmpad = None
                    indices_for_logits = indices

                # Memory optimization (2026-07-28): output_hidden_states=True installs
                # transformers' own capturing hooks (transformers/utils/output_capturing.py,
                # confirmed by reading it) that unconditionally hold ALL 33 layers' FULL
                # (1, total_nnz, hidden) tensors simultaneously in output.hidden_states --
                # only sliced down to response positions much later by
                # hidden_states_tuple_to_response_repr below, after the whole tuple has
                # already sat in memory for the rest of the forward pass. For
                # rep_distillation_layers="all" this is ~32 x total_nnz x hidden x 2 bytes.
                # Every layer's output ultimately only feeds hidden_states_tuple_to_response_
                # repr's own `hidden_states[:, -response_len:, :]` slice (see
                # extract_response_hidden_states in rep_distillation.py) -- confirmed by
                # reading that function, not assumed. Registering our OWN forward hooks that
                # slice each layer's output down to just those rows THE MOMENT it's produced
                # (discarding the rest immediately, one small tensor at a time) computes the
                # same final result without ever holding 32 full copies at once. Left disabled
                # (falls back to the old, unoptimized, already-correct output_hidden_states=
                # True path) for Ulysses SP (unused by this worker) and for
                # rep_distillation_layers="last" (already touches only one layer).
                # Narrowed to layers="all" specifically (not just != "last"): our hook captures
                # EVERY Qwen3_5DecoderLayer unconditionally, which is exactly what "all" needs;
                # "even"/"odd" would need extra filtering this path doesn't implement and isn't
                # exercised by our config (rep_distillation_layers=all), so they still fall
                # through to the old, unoptimized-but-correct path below.
                use_hook_hidden_states = (
                    return_last_hidden_repr and rep_distillation_layers == "all" and not self.use_ulysses_sp
                )
                hidden_keep_mask_rmpad = None
                indices_for_hidden = None
                hook_handles = []
                hook_results = None
                if use_hook_hidden_states:
                    response_len = response_mask.size(1)
                    hidden_keep_mask = torch.zeros(batch_size, seqlen, dtype=torch.bool, device=input_ids.device)
                    hidden_keep_mask[:, -response_len:] = True
                    hidden_keep_mask_rmpad = index_first_axis(
                        rearrange(hidden_keep_mask.unsqueeze(-1), "b s ... -> (b s) ..."), indices
                    ).squeeze(-1)  # (total_nnz,) bool
                    indices_for_hidden = indices[hidden_keep_mask_rmpad]

                    from transformers.models.qwen3_5.modeling_qwen3_5 import Qwen3_5DecoderLayer

                    hook_layers = [m for m in self.reward_module.modules() if isinstance(m, Qwen3_5DecoderLayer)]
                    # Bug fix (2026-07-29): for the multimodal Qwen3_5Config, num_hidden_layers
                    # lives on the nested text_config (Qwen3_5TextConfig), not on the top-level
                    # config directly (confirmed by reading configuration_qwen3_5.py -- Qwen3_5
                    # Config.sub_configs = {"vision_config": ..., "text_config": Qwen3_5TextConfig}
                    # and num_hidden_layers is only defined on Qwen3_5TextConfig). The original
                    # getattr(config, "num_hidden_layers", None) silently returned None here,
                    # which unconditionally tripped this safety net every time -- confirmed via a
                    # real 8-GPU run whose traceback showed the OLD output_hidden_states=True
                    # path executing (fsdp_workers.py:2466) despite this branch's preconditions
                    # all holding, meaning the hook path had never actually engaged on real
                    # hardware in any prior test. Check text_config first, fall back to the
                    # top-level config for (hypothetical) plain text-only models.
                    _reward_config = getattr(self.reward_module, "module", self.reward_module).config
                    _text_config = getattr(_reward_config, "text_config", None) or _reward_config
                    expected_num_layers = getattr(_text_config, "num_hidden_layers", None)
                    if expected_num_layers is None or len(hook_layers) != expected_num_layers:
                        # Safety net: if the discovered layer count doesn't match the config,
                        # don't silently capture the wrong thing -- fall back to the old,
                        # unoptimized-but-correct output_hidden_states=True path instead.
                        use_hook_hidden_states = False
                        hidden_keep_mask_rmpad = None
                        indices_for_hidden = None
                    else:
                        hook_results = [None] * len(hook_layers)

                        def _make_hidden_state_hook(layer_idx):
                            def _hook(module, args, output):
                                layer_hidden = output.squeeze(0) if output.dim() == 3 else output
                                hook_results[layer_idx] = layer_hidden[hidden_keep_mask_rmpad].detach()

                            return _hook

                        hook_handles = [
                            layer.register_forward_hook(_make_hidden_state_hook(i))
                ...
                if self.use_ulysses_sp:
                    # Bug fix (2026-07-28): this branch is inside RewardModelWorker, which has no
                    # `actor_module` attribute (that's ActorRolloutRefWorker's name for its model;
                    # RewardModelWorker's is `reward_module`, used two lines below in the same
                    # function) -- AttributeError, confirmed via a real crash the first time this
                    # worker was ever run with use_ulysses_sp=True (multimodal + sequence-parallel
                    # reward model is a previously-unexercised combination in this codebase).
                ...
                if use_logits_to_keep:
                    # keep the labels aligned with the (now response-position-only) logits
                    input_ids_rmpad_rolled = input_ids_rmpad_rolled[keep_mask_rmpad]
                try:
                    output = self.reward_module(
                        ...
                        **multi_modal_inputs,
                        **(
                            {"mm_token_type_ids": mm_token_type_ids_rmpad}
                            if mm_token_type_ids_rmpad is not None
                            else {}
                        ),
                        **({"logits_to_keep": keep_mask_rmpad} if use_logits_to_keep else {}),
                        use_cache=False,
                        return_dict=True if return_last_hidden_repr else self.use_fused_kernels,
                        output_hidden_states=return_last_hidden_repr and not use_hook_hidden_states,
                    )
                finally:
                ...
                if return_last_hidden_repr:
                    ...
                    if use_hook_hidden_states:
                        # TEMP DIAGNOSTIC (2026-07-29, remove once confirmed on real hardware):
                        # never fired in any prior run -- root-caused to the text_config lookup
                        # bug fixed above (this branch's own preconditions were never actually
                        # satisfied before). The standalone synthetic-data test already proved
                        # the per-layer indexing math bit-identical to the old path; this checks
                        # the one thing that test couldn't -- real hooks on the real FSDP-wrapped
                        # layers actually firing.
                        _none_layers = [i for i, r in enumerate(hook_results) if r is None]
                        print(
                            f"[hidden_hook DIAG] num_layers={len(hook_results)} "
                            f"none_layers={_none_layers} "
                            f"shapes={[tuple(r.shape) if r is not None else None for r in hook_results[:3]]}... "
                            f"kept={indices_for_hidden.shape[0]}/{indices.shape[0]}",
                            flush=True,
                        )
                        # Bug fix (2026-07-29): the previous version built a full 32-entry list
                        # of (batch, seqlen, hidden) reconstructions BEFORE calling
                        # hidden_states_tuple_to_response_repr -- since a Python tuple/list keeps
                        # every element alive for its whole lifetime regardless of the callee's
                        # own per-layer loop, this held all 32 full-size tensors simultaneously
                        # at the moment that function ran, defeating the whole point (confirmed
                        # by a real 8-GPU OOM landing exactly inside that function's per-layer
                        # .float() call, at 23.61/23.64GiB in use with only 1.5MiB free -- i.e.
                        # everything ELSE now fits, only this last step doesn't). Fixed by
                        # reconstructing, extracting, and discarding ONE layer at a time using
                        # the same unchanged extract_single_layer/stack/compact helpers
                        # hidden_states_tuple_to_response_repr itself uses internally, so at most
                        # one full-size tensor is ever alive. hook_results already contains
                        # exactly the 32 real transformer layers (no embeddings slot to skip --
                        # only Qwen3_5DecoderLayer instances get hooked), so no layer_indices/
                        # off-by-one bookkeeping is needed here.
                        from verl.utils.rep_distillation import (
                            _extract_single_layer_response_repr,
                            compact_response_repr_by_positions,
                            stack_layer_response_reprs,
                        )

                        layer_reprs = []
                        for sliced in hook_results:
                            full_layer_hidden = pad_input(
                                hidden_states=sliced,
                                indices=indices_for_hidden,
                                batch=batch_size,
                                seqlen=seqlen,
                            )
                            layer_reprs.append(
                                _extract_single_layer_response_repr(
                                    full_layer_hidden.float(), response_mask, rep_distillation_positions
                                )
                            )
                            del full_layer_hidden
                        teacher_last_hidden_repr = stack_layer_response_reprs(layer_reprs)
                        if rep_distillation_positions in ("first_k", "last_k"):
                            teacher_last_hidden_repr, _ = compact_response_repr_by_positions(
                    ...
                    elif rep_distillation_layers != "last":
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        # Multimodal support (2026-07-28): this worker is used as a multimodal teacher for
        # on-policy rep-distillation, but rm_data was built above from bare tensor fields only --
        # data.non_tensor_batch["multi_modal_inputs"] (per-sample pixel_values/image_grid_thw/etc)
        # was silently dropped, so the "teacher" never actually saw the video. rm_data.batch is a
        # TensorDict with the same row count as this data, so assigning it onto a batch key makes
        # tensordict auto-wrap it as a NonTensorStack that rearrange_micro_batches/.split() (both
        # called on rm_data.batch below) slice row-aligned along with input_ids/position_ids/etc
        # automatically, same as any other per-row key -- BUT only if it's a plain Python list.
        # verl's non_tensor_batch stores this as a numpy object array, and TensorDict treats a
        # numpy array as ONE opaque non-tensor blob (not one item per row) rather than unpacking
        # it -- confirmed by direct testing against the live tensordict version, not guessed
        # (assigning the numpy array as-is silently produced identical, unsliced, all-rows data on
        # every micro-batch regardless of which rows it should have contained). list(...) forces
        # the correct per-row wrapping.
        if "multi_modal_inputs" in data.non_tensor_batch.keys():
            rm_data.batch["multi_modal_inputs"] = list(data.non_tensor_batch["multi_modal_inputs"])
        if "mm_token_type_ids" in data.batch.keys():
            rm_data.batch["mm_token_type_ids"] = data.batch["mm_token_type_ids"]
