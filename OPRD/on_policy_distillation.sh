...
export ACTOR_MODEL_PATH=/root/models/Qwen3.5-4B
...
# export REWARD_MODEL_PATH=${MODEL_DIR}/JustRL-DeepSeek-1.5B
export REWARD_MODEL_PATH=/root/models/Qwen3.5-9B
...
python3 -m verl.trainer.main_ppo \
    ...
    data.return_multi_modal_inputs=${RETURN_MULTI_MODAL_INPUTS:-False} \
    ${IMAGE_PATCH_SIZE:+data.image_patch_size=$IMAGE_PATCH_SIZE} \
    ...
    actor_rollout_ref.actor.ppo_max_token_len_per_gpu=${ACTOR_PPO_MAX_TOKEN_LEN_PER_GPU:-$PPO_MAX_TOKEN_LEN_PER_GPU} \
    actor_rollout_ref.actor.ulysses_sequence_parallel_size=$PARALLEL_SIZE \
    $KL_ARGS \
    actor_rollout_ref.actor.loss_agg_mode=$LOSS_AGG_MODE \
    actor_rollout_ref.actor.entropy_from_logits_with_chunking=${ENTROPY_CHUNKING:-True} \
    actor_rollout_ref.actor.entropy_checkpointing=${ENTROPY_CHECKPOINTING:-True} \
    ...
    ${REWARD_N_GPUS_PER_NODE:+reward_model.n_gpus_per_node=$REWARD_N_GPUS_PER_NODE} \
    ${REWARD_NNODES:+reward_model.nnodes=$REWARD_NNODES} \
    ...
    reward_model.micro_batch_size_per_gpu=${REWARD_MICRO_BATCH_SIZE_PER_GPU:-24} \
    reward_model.use_dynamic_bsz=${REWARD_USE_DYNAMIC_BSZ:-False} \
    ${REWARD_FORWARD_MAX_TOKEN_LEN_PER_GPU:+reward_model.forward_max_token_len_per_gpu=$REWARD_FORWARD_MAX_TOKEN_LEN_PER_GPU} \
    reward_model.ulysses_sequence_parallel_size=${REWARD_ULYSSES_SP:-1} \
    custom_reward_function.path="${REWARD_FUNCTION_PATH:-verl/utils/reward_score/ttrl_math/__init__.py}" \
    custom_reward_function.name=${REWARD_FUNCTION_NAME:-reward_func} \
